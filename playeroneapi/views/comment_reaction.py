...
class CommentReactionView(ViewSet):
    ...
    def update(self, request, pk):
        ...
        comment_reaction.comment = Comment.objects.get(
            pk=request.data['commentId'])
        comment_reaction.reaction = Reaction.objects.get(
            pk=request.data['reactionId'])
...
class ReactionJointView(generics.ListCreateAPIView):
    serializer_class = CommentReactionSerializer

    def get_queryset(self):
        reaction = self.kwargs['reaction_id']
        return CommentReaction.objects.filter(reaction__id=reaction)
